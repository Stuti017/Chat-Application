...
function Home() {
  ...
  const [currentList, setCurrentList] = useState("My Chats");

  const handleDropdownChange = async(e) =>{
    setCurrentList(e.target.value);
    const currentList = e.target.value;
    if(currentList === 'My Chats'){
      setSearch("");
      setSearchResults([]);
      return;
    }
    else {
      if(currentList === 'Alumni'){
        setIsLoading(true)
        setSearch(" ");
        const { data } = await searchUsers(" ")
        const alumniUsers = data.filter(user => user.accountType === 'Alumni');
        setSearchResults(alumniUsers);
        setIsLoading(false)
      }
      else if(currentList === 'Professor'){
        setIsLoading(true)
        setSearch(" ");
        const { data } = await searchUsers(" ")
        const profUsers = data.filter(user => user.accountType === 'Professor');
        setSearchResults(profUsers);
        setIsLoading(false)
      }
    }
  }
    ...
    <>
      ...
      <div className="bg-[#282C35!] scrollbar-hide z-10 h-[100vh]  lg:w-[90%] lg:mx-auto overflow-y-hidden shadow-2xl">
        ...
        <div className='flex'>
          {
            !showProfile ?
              <div className="md:flex md:flex-col min-w-[360px] h-[100vh] md:h-[100vh] relative bg-white">
                <div className='h-[55px] px-4 bg-white'>
                  <div className='flex items-center relative -top-4 h-[90px]'>
                ...
                <div>
                  ...
                  <div className=' relative py-3 px-4'>
                    <form onSubmit={(e) => e.preventDefault()} className='flex w-[99.5%] bg-[#f6f6f6] text-[#111b21] tracking-wider py-[8px] ps-3 pe-5 rounded-[9px] outline-0'>

                      <BsSearch style={{ color: "#c4c4c5" }} className='my-auto mx-5'/>
                      <input value={search} onChange={handleSearch} className='bg-[#f6f6f6]' type="text" name="search" placeholder="Search" autoComplete='off'/>
                    ...
                    <div style={{ display: search ? "" : "none" }} className='h-[100vh] absolute z-10 w-[100%] left-[0px] top-[70px] bg-white flex flex-col gap-y-3 pt-3 px-4'>
